...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-gray-900 text-white flex flex-col flex-shrink-0">
        <div class="h-16 flex items-center px-6 border-b border-gray-700">
            <span class="text-lg font-bold tracking-wide">Altum Credo</span>
            <span class="ml-2 text-xs text-gray-400">ITSM</span>
        </div>
        <nav class="flex-1 overflow-y-auto py-4">
            <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-6 py-2.5 text-sm hover:bg-gray-700 {{ request()->routeIs('dashboard') ? 'bg-gray-700 text-white' : 'text-gray-300' }}">
            ...
            <a href="{{ route('tickets.index') }}" class="flex items-center gap-3 px-6 py-2.5 text-sm hover:bg-gray-700 {{ request()->routeIs('tickets.*') ? 'bg-gray-700 text-white' : 'text-gray-300' }}">
            ...
            <a href="{{ route('tickets.create') }}" class="flex items-center gap-3 px-6 py-2.5 text-sm hover:bg-gray-700 text-gray-300">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                New Ticket
            </a>
            @if(auth()->user()->isLead() || auth()->user()->isAdmin() || auth()->user()->isMd())
            <a href="{{ route('team.index') }}" class="flex items-center gap-3 px-6 py-2.5 text-sm hover:bg-gray-700 {{ request()->routeIs('team.*') ? 'bg-gray-700 text-white' : 'text-gray-300' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0"/></svg>
                Team
            </a>
            <a href="{{ route('reports.index') }}" class="flex items-center gap-3 px-6 py-2.5 text-sm hover:bg-gray-700 {{ request()->routeIs('reports.*') ? 'bg-gray-700 text-white' : 'text-gray-300' }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                Reports
            </a>
            @endif
            @if(auth()->user()->isAdmin())
            <div class="mt-4 px-6 py-1 text-xs text-gray-500 uppercase tracking-wider">Admin</div>
            <a href="{{ route('admin.users.index') }}" class="flex items-center gap-3 px-6 py-2.5 text-sm hover:bg-gray-700 {{ request()->routeIs('admin.users.*') ? 'bg-gray-700 text-white' : 'text-gray-300' }}">
            ...
            <a href="{{ route('admin.categories.index') }}" class="flex items-center gap-3 px-6 py-2.5 text-sm hover:bg-gray-700 {{ request()->routeIs('admin.categories.*') ? 'bg-gray-700 text-white' : 'text-gray-300' }}">
